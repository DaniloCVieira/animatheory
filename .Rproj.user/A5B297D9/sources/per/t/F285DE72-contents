...
plotrichness<-function(species_table,pool_table,time=0, n_sp=length(table(na.omit(species_table[,3]))), title2='Community',cycles=100,main)
{
  ...
  { plot(seq(0,cycles, length.out=cycles),seq(0,max(n_sp), length.out=cycles), type="n", xlab="Time steps", ylab="Number of species", bty="l", ylim=c(0,nrow(pool_table)), las=1)
    title("Species Richness",font.main=1,cex.main=1.5, line=2)
    if(length(time)==1){
...
plotrank<-function(species_table,pool_table,time=0, n_sp=length(table(na.omit(species_table[,3]))), title2='Community',cycles=100,main)
{
  ...
  par(xpd=F)
  par(mar=c(5.1, 4.1, 9, 2.1))
  plot(sprank,type='b', axes=F, xlab="species abundance rank", ylab="number of species", ylim=c(0,100), pch=names(sprank),col=colrank)
  title("Rank abundance",font.main=1,cex.main=1.5, line=2)
  axis(1, at=c(1:length(sprank)), labels=c(1:length(sprank)))
    axis(2, las=1, at=c(0,20,40,60,80,100), labels=c(0,20,40,60,80,100))
  par(xpd=T)

  mtext(main, side = 3, line = -3, outer = TRUE,cex=2.5,font=2)
  mtext("powered by Vieira, D. C.", side = 1, line = -1, outer = TRUE,cex=.8,font=3)
  par(xpd=F)
  }
}
...

{
...

  {
    ...
    par(xpd=F, mar=c(2, 1, 6, 0), cex=1.7)
    ...
    plot( positions[,1], positions[,2], xlim=lims, ylim=lims, pch=c(species), cex=1.7, col=cols, ann=F, axes=F, type="n")
    title(title2,font.main=1, line=-1,cex.main=1.5)


    par(xpd=T)
    points(rep(-.5,nrow(pool_table)),seq(min(species_table[,2])-2,max(species_table[,2])+4,length.out=(nrow(pool_table))), pch=pool_table[,3],col=pool_table[,4], cex=1)
    par(xpd=F)
    if(ncol(species_table)!=4)
    {
      ...
      par(xpd=T,cex=1)
      points(rep(-.5,nrow(pool_table)),seq(min(species_table[,2])-2,max(species_table[,2])+4,length.out=(nrow(pool_table))), pch=pool_table[,3],col=pool_table[,4], cex=1.2)
    ...
    points( positions[,1], positions[,2]-0.2, pch=c(species), cex=1.7, col=cols)
...
die<-function(species_table,pool_table,prob=0.1, cycles,main, xdim=xdim, ydim=ydim, n_pool=n_pool)
{
  ...
  points(species_table[ which(is.na(species_table[,3])),1:2], pch=  16, cex=1.7, col="black")
  plotrichness(species_table,pool_table,title2="Start community", cycles=cycles, main=main)
  plotrank(species_table,pool_table,title2="Start community", cycles=cycles, main=main)
  attr(species_table,"seed")<-seed0
...
#' @export
selection<-function(species_table,pool_table,selprob, cycles,main, xdim, ydim, n_pool,selforce)
{

  repeat {
    if(is.null(selforce)==F){
    sampled.rows<-order(selprob, decreasing = T)[1:selforce]}
     else {sampled.rows<-which(selprob>=0.5)}
...
#' @export
simulate<-function(species_table,pool_table, cycles, actions=c("death","birth","immig","disp"),prob=NULL,env_selection=F,random_death=T,sel=100,  bd=.505, path=NULL,displim=T, main="anima_theory", record=T, xdim, ydim, n_pool,selforce)
{
  seed0<-attr(species_table,"seed")+1
  if(is.null(prob)){
    die.prob<-0.1
  ...
  for (t in 1:cycles)
  {
    ...
    if(isTRUE(env_selection)){

      {
        ...
        species_table[,'probdie']<-probdie
        if(is.null(path)==F){   png(paste0(path,recordpic(path)) , 500, 280)}
        par(mfrow=c(1,3),mar=c(5.1, 4.1, 9, 2.1), cex=1.25)
        species_table<-selection(species_table,pool_table, selprob=probdie, cycles=cycles,main=main, xdim=xdim, ydim=ydim,n_pool=n_pool,selforce=selforce)
...
#' @export
maketheory<-function(filename,n_sp=25,cycles=100, build_envi = F,npatchs=8,  actions=c("death","birth","immig","disp"),prob=c(0.01,0.6,0.4,0.1), env_selection = F,random_death=T, sel=.3,displim = T,main="anima_theory",xdim=5,ydim=5, n_pool=35, seed0,selforce){
  ...
  species_table<-simulate(species_table,pool_table, cycles=cycles, actions=c(actions),prob=c(prob), env_selection =env_selection,random_death=random_death, sel=sel,path=NULL,displim = displim, xdim=xdim, ydim=dim, n_pool=n_pool,selforce=selforce)
...
#' @export
simulatheory<-function(n_sp=25,cycles=100, build_envi = T,npatchs=8,  actions=c("death","birth","immig","disp"),prob=c(0.01,0.6,0.4,0.1), env_selection = T,random_death=F, sel=.3,displim = T,path=NULL,main="anima_theory",xdim=5,ydim=5, n_pool=35, seed0,selforce){

  init<-initiate(n_sp=n_sp,xdim=xdim,ydim=ydim, n_pool=n_pool,emoj_cat="Animals & Nature", cycles=cycles, build_envi=build_envi, npatchs=npatchs,sel=sel, main=main, seed0 = seed0)
  species_table<-init$sp
  pool_table<-init$pool
  species_table<-simulate(species_table,pool_table, cycles=cycles, actions=c(actions),prob=c(prob), env_selection =env_selection,random_death=random_death, sel=sel,path=path,displim = displim, main=main, xdim=xdim, ydim=dim, n_pool=n_pool,selforce=selforce)
  }

#' @export
animatheory0<-function(title,n_sp=25,cycles=100, build_envi = F,npatchs=8, actions=c("death","birth","immig","disp"),prob=c(0.01,0.6,0,0), env_selection = F,random_death=T, sel=.3,displim = F, path=NULL,main="anima_theory", xdim, ydim, n_pool, seed0,selforce=NULL) {
  des="\n\n\n\n powered by Vieira, D. C."
  saveHTML({


    ani.options(verbose=F)
    simulatheory(n_sp=n_sp,cycles=cycles, build_envi = build_envi,npatchs=npatchs, actions=c(actions),prob=c(prob), env_selection = env_selection,random_death=random_death, sel=sel,displim = displim, path=path, main=main, xdim=xdim,ydim=ydim, n_pool=n_pool, seed0=seed0,selforce=selforce)

  }, img.name = title, imgdir = title, htmlfile = paste0(title,".html"),
  autobrowse = T,interval = 0.2,ani.height = 600, ani.width = 1400, loop=1, autoplay = FALSE,verbose=F,description = des, loop=F)


}

args<-list(title=title,n_sp=10,cycles=60, build_envi = F,npatchs=5, actions=c("death","birth","immig","disp"),prob=c(10,0.6,0,0), env_selection = F,random_death=T, sel=.3,displim = F, path=NULL, main=main, xdim=7,ydim=7,n_pool=25, seed0=5)


#' @export
animatheory<-function(title,n_sp=25,cycles=100, build_envi = F,npatchs=8, actions=c("death","birth","immig","disp"),prob=c(0.01,0.6,0,0), env_selection = F,random_death=T, sel=.3,displim = F, path=NULL,main="anima_theory", xdim, ydim, n_pool, seed0,selforce=NULL) {
  des="\n\n\n\n powered by Vieira, D. C."
  saveHTML({
  ...
  autobrowse = T,interval = 0.1,ani.height = 550, ani.width = 1300, loop=1, autoplay = FALSE,verbose=F,description = des, loop=F)
